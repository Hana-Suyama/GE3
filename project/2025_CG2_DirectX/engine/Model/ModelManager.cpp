...
void ModelManager::CreateSkyBox()
{
	...
	//球の頂点リソース
	const uint32_t vertexCount = static_cast<uint32_t>(newModel.meshes_.at(0).vertices.size());
	newModel.meshes_.at(0).vertexResource = directXBasic_->CreateBufferResource(sizeof(VertexData) * vertexCount);
	//リソースの先頭のアドレスから使う
	newModel.meshes_.at(0).vertexBufferView.BufferLocation = newModel.meshes_.at(0).vertexResource->GetGPUVirtualAddress();
	//使用するリソースのサイズは分割数×分割数×6のサイズ
	newModel.meshes_.at(0).vertexBufferView.SizeInBytes = sizeof(VertexData) * vertexCount;
	//1頂点当たりのサイズ
	newModel.meshes_.at(0).vertexBufferView.StrideInBytes = sizeof(VertexData);
	////球の頂点リソースにデータを書き込む
	VertexData* vertexDataSphere = nullptr;
	...
	//球用のindexリソース
	newModel.meshes_.at(0).indexResource = directXBasic_->CreateBufferResource(sizeof(uint32_t) * vertexCount);
	//リソースの先頭のアドレスから使う
	newModel.meshes_.at(0).indexBufferView.BufferLocation = newModel.meshes_.at(0).indexResource->GetGPUVirtualAddress();
	//使用するリソースのサイズはインデックス*vertexTotalNumberのサイズ
	newModel.meshes_.at(0).indexBufferView.SizeInBytes = UINT(sizeof(uint32_t) * vertexCount);
	//インデックスはuint32_tとする
	newModel.meshes_.at(0).indexBufferView.Format = DXGI_FORMAT_R32_UINT;
	//球用インデックスリソースにデータを書き込む
