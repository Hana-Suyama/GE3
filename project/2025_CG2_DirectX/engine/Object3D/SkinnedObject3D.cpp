...
void SkinnedObject3D::Initialize(SkinnedObject3DBasic* skinnedObject3DBasic, ModelManager* modelManager, std::string modelFilePath)
{
	...
	camera_ = skinnedObject3DBasic_->GetDefaultCamera();
...
void SkinnedObject3D::Draw()
{
	...
	if (isDraw_) {
		...
		for (int32_t i = 0; i < modelData_->meshes_.size(); i++) {
			...
			// マテリアルCBufferの場所を設定
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootConstantBufferView(0, materialResources_.at(i)->GetGPUVirtualAddress());
			// IBVを設定
...
void SkinnedObject3D::DrawSkeletonDebug()
{
	...
	for (int32_t jointIndex = 0; jointIndex < skeleton_.joints.size(); jointIndex++) {
		...
		for (int32_t i = 0; i < skeletonDebugSphereModelData_->meshes_.size(); i++) {
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootDescriptorTable(2, modelManager_->GetTextureManager()->GetSrvHandleGPU(skeletonDebugTextureFilePaths_.at(i)));
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootDescriptorTable(5, modelManager_->GetTextureManager()->GetSrvHandleGPU(cubeTextureFilePaths_));
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->IASetVertexBuffers(0, 1, &skeletonDebugSphereModelData_->meshes_.at(i).vertexBufferView);
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootConstantBufferView(1, skeletonDebugTransformationMatrixResources_[jointIndex]->GetGPUVirtualAddress());
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootConstantBufferView(0, skeletonDebugMaterialResources_.at(i)->GetGPUVirtualAddress());
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->IASetIndexBuffer(&skeletonDebugSphereModelData_->meshes_.at(i).indexBufferView);
			skinnedObject3DBasic_->GetDirectXBasic()->GetCommandList()->DrawIndexedInstanced(UINT(skeletonDebugSphereModelData_->meshes_.at(i).vertices.size()), 1, 0, 0, 0);
		}
	}
}

void SkinnedObject3D::DebugDraw(std::string label)
{
#ifdef USE_IMGUI
...
void SkinnedObject3D::SetModelData(std::string modelFilePath)
{
...
void SkinnedObject3D::CreateWVPResource()
{
	...
	transformationMatrixResource_ = skinnedObject3DBasic_->GetDirectXBasic()->CreateBufferResource(sizeof(TransformationMatrix));
	//データを書き込むためのアドレスを取得
...
void SkinnedObject3D::CreateSkeletonDebugResources()
{
	...
	for (int32_t i = 0; i < skeletonDebugSphereModelData_->meshes_.size(); i++) {
		Microsoft::WRL::ComPtr<ID3D12Resource> materialResource = skinnedObject3DBasic_->GetDirectXBasic()->CreateBufferResource(sizeof(Material));
...
void SkinnedObject3D::CreateMTUV()
{
