...
void SkyBox::Initialize(SkyBoxBasic* skyBoxBasic, ModelManager* modelManager, std::string modelFilePath)
{
	skyBoxBasic_ = skyBoxBasic;
	...
	camera_ = skyBoxBasic_->GetDefaultCamera();
...
void SkyBox::Draw()
{
	...
	if (isDraw_) {
		...
		for (int32_t i = 0; i < modelData_->meshes_.size(); i++) {
			// テクスチャを設定
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootDescriptorTable(2, modelManager_->GetTextureManager()->GetSrvHandleGPU(textureFilePaths_.at(i)));
			// VBVを設定
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->IASetVertexBuffers(0, 1, &modelData_->meshes_.at(i).vertexBufferView);
			// wvp用のCBufferの場所を設定
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootConstantBufferView(1, transformationMatrixResource_->GetGPUVirtualAddress());
			// マテリアルCBufferの場所を設定
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->SetGraphicsRootConstantBufferView(0, materialResources_.at(i)->GetGPUVirtualAddress());
			// IBVを設定
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->IASetIndexBuffer(&modelData_->meshes_.at(i).indexBufferView);
			// 描画！ (DrawCall/ドローコール)。3頂点で1つのインスタンス。インスタンスについては今後
			skyBoxBasic_->GetDirectXBasic()->GetCommandList()->DrawIndexedInstanced(UINT(modelData_->meshes_.at(i).vertices.size()), 1, 0, 0, 0);
		}

	}
}

void SkyBox::DebugDraw(std::string label)
{
#ifdef USE_IMGUI
...
void SkyBox::SetModelData(std::string modelFilePath)
{
...
void SkyBox::CreateWVPResource()
{
	...
	transformationMatrixResource_ = skyBoxBasic_->GetDirectXBasic()->CreateBufferResource(sizeof(TransformationMatrix));
	//データを書き込むためのアドレスを取得
...
void SkyBox::CreateMTUV()
{
	...
	for (int32_t i = 0; i < modelData_->meshes_.size(); i++) {
		...
		Microsoft::WRL::ComPtr<ID3D12Resource> materialResource = skyBoxBasic_->GetDirectXBasic()->CreateBufferResource(sizeof(Material));
