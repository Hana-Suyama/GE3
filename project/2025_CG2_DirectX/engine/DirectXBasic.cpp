...
void DirectXBasic::CreateCommand()
{
	...
	HRESULT hr = device_.Get()->CreateCommandQueue(&commandQueueDesc, IID_PPV_ARGS(&commandQueue_));
...
void DirectXBasic::CreateSwapChain()
{
	...
	HRESULT hr = dxgiFactory_->CreateSwapChainForHwnd(commandQueue_.Get(), winApi_->GetHwnd(), &swapChainDesc_, nullptr, nullptr, reinterpret_cast<IDXGISwapChain1**>(swapChain_.GetAddressOf()));
	assert(SUCCEEDED(hr));
}
...
Microsoft::WRL::ComPtr<ID3D12Resource> DirectXBasic::CreateRenderTextureResource(Comptr<ID3D12Device> device, uint32_t width, uint32_t height, DXGI_FORMAT format, const Vector4& clearColor)
{
	...
	resourceDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;	//レンダーターゲットとして利用するフラグ
	...
	HRESULT hr = device_->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_RENDER_TARGET,	//Resourceの初期状態。レンダーターゲットとして利用するので、レンダーターゲットの状態にしておく
		&clearValue,	//Clear最適値
...
void DirectXBasic::CreateRenderTexturePSO()
{
	...
	hr = device_->CreateRootSignature(0,
		...
		IID_PPV_ARGS(&rootSignatureRenderTexture_));
	assert(SUCCEEDED(hr));
	...
	//Shaderをコンパイルする
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob = CompileShader(L"resources/shaders/Dissolve.VS.hlsl",
		L"vs_6_0", logger_);
	assert(vertexShaderBlob != nullptr);

	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = CompileShader(L"resources/shaders/Dissolve.PS.hlsl",
		L"ps_6_0", logger_);
	assert(pixelShaderBlob != nullptr);

	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateDesc{};
	graphicsPipelineStateDesc.pRootSignature = rootSignatureRenderTexture_.Get();//RootSignature
...
void DirectXBasic::CreateRTV()
{
	...
	// オフスクリーンレンダリング用のテクスチャを作る
	renderTextureResource_ = CreateRenderTextureResource(device_, WindowsApi::kClientWidth, WindowsApi::kClientHeight,
...
void DirectXBasic::PreDrawRenderTexture()
{
	...
	//指定した色で画面全体をクリアする
	commandList_->ClearRenderTargetView(rtvHandles_[2], reinterpret_cast<const float*>(&kRenderTargetClearValue), 0, nullptr);
	//指定した深度で画面全体をクリアする
	commandList_->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);
...
void DirectXBasic::PreDrawBackBuffer()
{
	...
	//指定した色で画面全体をクリアする
	float clearColor[] = { 0.0f, 1.0f, 0.0f, 1.0f };	//青っぽい色。RGBAの順
	GetCommandList()->ClearRenderTargetView(GetRtvHandle(backBufferIndex), clearColor, 0, nullptr);

	GetCommandList()->RSSetViewports(1, &GetViewport());	//Viewportを設定
	GetCommandList()->RSSetScissorRects(1, &GetScissorRect());	//Scissorを設定
}
