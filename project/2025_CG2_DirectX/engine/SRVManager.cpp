...
void SRVManager::CreateSRVforStructuredBuffer(uint32_t srvIndex, ID3D12Resource* pResource, UINT numElements, UINT structureByteStride)
{
	...
	directXBasic_->GetDevice()->CreateShaderResourceView(pResource, &instancingSrvDesc, instancingSrvHandleCPU);

	directXBasic_->GetDevice()->CreateShaderResourceView(pResource, &instancingSrvDesc, GetCPUDescriptorHandle(srvIndex));
}


void SRVManager::CreateSRVforRendertargetTexture(uint32_t srvIndex, ID3D12Resource* pResource, DXGI_FORMAT Format)
{
	...
	// SRVの生成
	directXBasic_->GetDevice()->CreateShaderResourceView(pResource, &renderTextureSrvDesc, GetCPUDescriptorHandle(srvIndex));
}
...
void SRVManager::CreateSRVforDepthTexture(uint32_t srvIndex, ID3D12Resource* pResource)
{
	...
	directXBasic_->GetDevice()->CreateShaderResourceView(pResource, &depthTextureSrvDesc, GetCPUDescriptorHandle(srvIndex));
}
